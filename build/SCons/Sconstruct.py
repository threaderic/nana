# !! The files Sconstruct.py and SconsLocalTools must be placed in the directory build/scons/ of the nana's directory !!
# --> cause: relatives paths used (it could be placed in another directory but all Paths would be modified.
...
# 3 build modes
vars = Variables()
vars.Add(EnumVariable('mode', 'Building mode', 'release', allowed_values=('debug', 'profile', 'release')))
...
if env['mode'] == 'debug':
    env.Append(CCFLAGS = ['-g', '-fexceptions', '-Wall', '-Wextra', '-Wunused-variable', '-Wfatal-errors', '-O0', '-DDEBUG'])
elif env['mode'] == 'release':
    SetOption('silent', True)                           # allow to compile in silent mode --> equivalent to: $ scons --silent
...
# env.Append(CCFLAGS='-std=c++17 -no-pie -g3 -Wall -Wextra -I/usr/local/include/ -I/usr/include/ -I. -I../../include/')
...
conf2 = Configure(env, custom_tests = { 'CheckPKGConfig' : SconsLocalTools.CheckPKGConfig,      # function define in SconsLocalTools.py
                                       'CheckPKG' : SconsLocalTools.CheckPKG                    # function define in SconsLocalTools.py
                                       })
...
# matches_cpp_files = ['../../source/unicode_bidi.cpp','../../source/any.cpp','../../source/basic_types.cpp']
...
# lib = env.Library('Library/nana', source = obj) --> library/libnana.a (relative to the Sconstruct file)
...
# copy also the library to the directory /build/bin ! (relative to the Sconstruct file)
path_to_bin = pathlib.Path.cwd().parent / 'bin'
lib_copy = env.Install(path_to_bin, lib)
env.AlwaysBuild(lib_copy)
# env.Default(lib_copy)                                   # using Alias could be another solution 
# env.Alias('install', '/usr/bin')                  # --> type: $ scons install 

#env.NoClean(lib_copy)                                  # --> if the library won't be erased with call: $ scons -c
...
finish_command = env.Command('finish', SconsLocalTools.finish(10,'release'), [])
...
if "env" in locals():
    SconsLocalTools.show_progress(env, 100, env['mode'], "Library Nana")  # 316, number of nodes calculated at end of compilation in debug mode and copied here
